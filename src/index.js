...
function isKey(jwk) {
    return (
        ...
        [ 'A256GCMKW', 'A256GCM', undefined ].includes(jwk.alg) &&
...
async function jwe(jwk) {
    ...
    async function decrypt(cipher) {
        const { value } = await jose.JWE
            .createDecrypt(key.keystore)
            .decrypt(cipher)
